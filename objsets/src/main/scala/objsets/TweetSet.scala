...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  // inefficient in run-time, immutable data structure plus recursion is a recipe for a disaster
  ...
  def mostRetweeted = {
    ...
    else if (right.isEmpty) maxRetweets(elem, left.mostRetweeted)
    else maxRetweets(elem, maxRetweets(left.mostRetweeted, right.mostRetweeted))
  }

  private def maxRetweets(a: Tweet, b: Tweet): Tweet = if (a.retweets > b.retweets) a else b
}
...
object GoogleVsApple {
  ...
  private def filterByKeywords(tweets: TweetSet, keywords: List[String]) =
    tweets.filter(tweet => keywords.exists(keyword => tweet.text.contains(keyword)))
