...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])], explored: Set[Block]): Stream[(Block, List[Move])] =
    if (initial.isEmpty) initial
      ...
      val moreExplored = explored ++ more.map(_._1)
      initial.head #:: from(initial.tail #::: more, moreExplored)
    }

  /**
   * The stream of all paths that begin at the starting block.
   */
  lazy val pathsFromStart: Stream[(Block, List[Move])] = from((startBlock, List.empty)#::Stream.empty, Set(startBlock))
